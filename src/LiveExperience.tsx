// LiveExperience — the R3F / live layer, lazy-split out of App so the entry
// chunk ships NO three.js (W6a perf slice). App statically imports only react,
// core/tiers (three-free feature detection), and StaticExperience (the DOM-only
// reduced-motion route); everything that touches @react-three/fiber, three/webgpu,
// the world/signature/flight/overture modules, the audio graph, the collapse
// preloader, and the mobile film lives HERE and streams in on demand.
//
// INVARIANT: this is a mechanical move from App.tsx — same component tree, same
// props, same mount order. The W4 overture wiring in ExperienceShell moved
// verbatim. Behavior after this chunk loads is identical to the pre-split tree;
// the only change is WHEN the chunk is fetched (lazy, on a live route — never on
// the static/reduced-motion route, never in the entry graph).
...
// The live layer's own router: collapse-harness (dev), mobile film, or the
// desktop live experience. App passes the resolved tier (+ 'collapse' dev flag);
// the coarse-mobile check lives here so App stays free of any three-touching
// import. dev === 'collapse' short-circuits before the mobile check (the harness
// renders on any device).
export default function LiveExperience({ tier, dev }: { tier: Tier; dev?: 'collapse' }) {
  if (dev === 'collapse') return <CollapseHarness tier={tier} />;
