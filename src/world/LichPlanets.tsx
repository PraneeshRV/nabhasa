...
// getPlanetPositions() — so Threshold's ~36 wu collider is provisioned when P1.3
...
// P3 approach trigger radius (wu). > planet radius, > courier.OFFER_RADIUS (60)
...
export const APPROACH_RADIUS = 100;
...
function AntennaFarm({ k, hull }: { k: number; hull: THREE.MeshStandardNodeMaterial }) {
  ...
  const count = Math.max(8, Math.round(base * k));
  const mesh = useMemo(() => {
    const geo = new THREE.CylinderGeometry(0.18, 0.3, 3.6, 5);
    const m = new THREE.InstancedMesh(geo, hull, count);
    const d = new THREE.Object3D();
    const dir = new THREE.Vector3();
    for (let i = 0; i < count; i++) {
      fibDir(i, count, dir);
      d.position.copy(dir).multiplyScalar(15.6); // just outside the torus
