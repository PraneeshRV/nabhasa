...
import { Conductor, HANDOVER_T, type OvertureBeatId } from './conductor';
import {
  createRail,
  frameWaypoints,
  railPointAt,
...
export function buildOvertureSources(): OvertureSources {
  return {
    ...
    spawn: new Vector3(SPAWN_POS[0], SPAWN_POS[1], SPAWN_POS[2]),
  };
}
...
export function Overture({
  ...
  useFrame((_, rawDt) => {
    ...
    try {
      ...
      // Drive the camera along the rail; look at the star/system center throughout
      // (the spawn faces the star, so this look-at hands off to flight seamlessly).
      camera.position.copy(railPointAt(railRef.current.curve, step.railParam));
      camera.lookAt(STAR_ORIGIN);

      // Emit newly-fired beats (future FX hook) + advance the HUD overlay state.
      for (const id of step.fired) onBeat?.(id);

      const hud: OvertureHudState = {
        fade: handoverFade(step.t),
