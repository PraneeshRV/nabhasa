...
describe('chaseSpawnPose — rail ends at the CHASE pose, not the craft pose (Finding 1)', () => {
  ...
  it('for the craftState initial equals the CameraRig chase pose (600,81.7,6), derived not magic', () => {
    // 6/1.7 restate cameraRig's OFFSET_BACK/OFFSET_UP (rescaled with the ×⅔ craft
    // shrink, re-fly round 2 2026-07-15) — kept literal so this test stays fiber-free.
    const chase = chaseSpawnPose(craftState.pos, craftState.forward, 6, 1.7);
    expect(chase.x).toBeCloseTo(craftState.pos.x, 6);
    expect(chase.y).toBeCloseTo(craftState.pos.y + 1.7, 6); // +up·1.7
    expect(chase.z).toBeCloseTo(craftState.pos.z + 6, 6); // forward (0,0,−1) ⇒ −forward·6 = +z·6
  });
